...
async function getVapidHeaders(audience, subject, publicKey, privateKey, contentEncoding, expiration) {
  ...
  const jwt = await new SignJWT(jwtPayload).setProtectedHeader(header).sign(new util.TextEncoder().encode(formatPEM(privateKey, "EC PRIVATE KEY")));
...
import { Buffer as Buffer3 } from "https://deno.land/std@0.141.0/node/buffer.ts";

// src/ece.ts
import { Buffer as Buffer2 } from "https://deno.land/std@0.141.0/node/buffer.ts";
...
import { encode as encode4 } from "https://cdn.skypack.dev/urlsafe-base64";

// src/web-push-error.ts
import util2 from "https://deno.land/std@0.146.0/node/util.ts";
...
util2.inherits(WebPushError, Error);
...
WebPushLib.prototype.generateRequestDetails = async function(subscription, payload, options) {
  ...
  if (payload) {
    ...
    if (contentEncoding === web_push_constants_default.supportedContentEncodings.AES_128_GCM) {
      ...
      requestDetails.headers["Crypto-Key"] = "dh=" + encode4(encrypted.localPublicKey);
    }
  ...
  if (isGCM) {
    ...
    const parsedUrl = parse2(subscription.endpoint);
